...
def data_task(sv, line, line_list, i):
    ...
    if '()' in line:
        ...
        struc_i[0], struc_i[1] = make_json(
            name, fix_argvs(remove_equal), type_, line)
    print(struc_i)
    return struc_i
    ...
    for sv in directory:
        ...
        with open(sv, 'r') as fp:
            for line in fp:
                ...
                if ((vtask in line_temp) or (vfuntion in line_temp)) and (
                        '(' in line_temp):  # + f
                    flag = 1
                if (((vtask in fix_line) or (vfuntion in fix_line)) and (
                        '(' in fix_line) and ((')' in fix_line) == False)) or flag:  # + f
                    fix_line = fix_line + line_temp
                if ((vtask in fix_line) or (vfuntion in fix_line)) and (
                        (('(' in fix_line) and (')' in fix_line)) or ('()' in fix_line)):  # + f
                    print('\n\n')
                    print(fix_line)
                    # we invoke functions
                    fix_line = delete_parenthesis(fix_line)
                    temp_list = separate(fix_line)
                    # we eliminate the fields before the word -function or
                    # task-
                    if (len(temp_list) >= 2) and (
                            (temp_list[1] == vfuntion) or (temp_list[1] == vtask)):
                        temp_list.pop(0)
                    else:
                        if (len(temp_list) >= 3) and (
                                (temp_list[2] == vfuntion) or (temp_list[1] == vtask)):
                            temp_list.pop(0)
                            temp_list.pop(0)
